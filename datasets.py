...
def get_test_dataset(args):
    ...
    if args.testset == 'flickr':
        ...
        testcsv = 'metadata/flickr_5k.csv'
    else:
    ...
    print(f"Number of img files: {len(image_files)}")
    print(f"Number of audio files: {len(image_files)}")
    avail_files = audio_files.intersection(image_files)
    print(f"Number of avail files: {len(testset)}")
    testset = testset.intersection(avail_files)
    print(f"Number of test files: {len(testset)}")
...
def get_test_dataset_2(args):
    ...
    # audio_path = args.test_data_path + 'audio/' #该加载方式有问题
    # image_path = args.test_data_path + 'frames/'
